...
class TestInferSchema:
    ...
    def test_bom_name_header(self):
        """BOM付きNameヘッダが正しくtitleとして扱われる"""
        path = self._write_csv("\ufeffName,Status\nAlice,Active\n")
    ...
    def test_whitespace_headers(self):
        """前後空白付きヘッダが正規化される"""
        path = self._write_csv("  Name  , Status ,  URL  \nA,B,https://x.com\n")
        schema = infer_schema(path)
        assert schema.columns[0].name == "Name"
...
class TestDatabaseSchemaConsistency:
    ...
    def test_bom_consistency(self):
        ...
        for row in rows:
            ...
            # 行のプロパティ名はすべてDBスキーマに含まれている
            for prop_name in props:
                assert prop_name in notion_schema, (
                    f"行のプロパティ '{prop_name}' がDBスキーマに存在しない。"
                    f" DB列: {list(notion_schema.keys())}"
                )

    def test_whitespace_consistency(self):
        """空白付きヘッダでも一致する"""
        path = self._write_csv("  Name  , Tags \nA,tag1\n")
        schema = infer_schema(path)
        notion_schema = schema.to_notion_schema()
